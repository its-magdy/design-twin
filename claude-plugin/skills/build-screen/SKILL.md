...
- **`codeconnect.local.json`** (repo root) — the component map: Figma component → your code component,
  ...
  it with `node tooling/map-bootstrap.js`, then fill in each entry's `code.module`/`code.export`.
...
- **`design/tokens.dtcg.json`** (generated by `node tooling/tokens.js`) plus **`design/tokens.json`** if
...
2. **Drift check.** Run the `design_drift_lint` MCP tool (or `node tooling/drift-lint.js`) FIRST — it
