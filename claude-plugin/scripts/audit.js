...
var require_cross_check = __commonJS({
  "design-to-code/cross-check.js"(exports2, module2) {
    ...
    function crossCheck(input) {
      ...
      const screenVarByName = /* @__PURE__ */ new Map();
      ...
      if (tokens && screenVarByName.size) {
        ...
        for (const c of collisions) {
          push(
            ...
            `'${c.name}'${c.alsoKnownAs ? ` and the design system's '${c.alsoKnownAs}'` : ""} share a name but resolve DIFFERENTLY: screen ${JSON.stringify(c.screen)} vs design system ${JSON.stringify(c.designSystem)}` + (Object.keys(c.screen).some((m) => m in c.designSystem) ? ". " : " \u2014 no mode name is shared and the two value sets are disjoint. ") + `Slugging the name onto the existing token would silently apply the wrong value \u2014 namespace the screen's copy, or confirm which library is authoritative.`,
    ...
    module2.exports = { crossCheck, toMarkdown: toMarkdown2, ABSURD_NUMBER, WRONG_CATALOG_PCT };
...
` : "") + USAGE);
      ...
      if (jsonOnly) {
        ...
        fs.writeFileSync(out + ".md", toMarkdown2(res));
        console.error(`wrote ${out}.json and ${out}.md`);
      } else {
        process.stdout.write(toMarkdown2(res));
      }
