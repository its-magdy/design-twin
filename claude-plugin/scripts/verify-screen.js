...
var TOLERANCE = {
  ...
  gap: 1,
  size: 2,
  // layout rounding across a whole row accumulates a pixel or two
  opacity: 0.02
};
...
function checkable(spec) {
  return ["text", "fontSize", "color", "backgroundColor", "borderRadius", "gap", "padding", "borderColor"].some((k) => spec[k] !== void 0);
}
function buildExpectation(docs) {
  ...
  return {
    ...
    exportedAt,
    reference,
    note: "Generated from the export \u2014 do NOT retype these numbers into code comments. Every row is the value the design states; a field the export does not define is absent rather than defaulted. Feed this to a renderer probe and compare with `verify-screen.js --compare`.",
    tolerance: TOLERANCE,
...
function compare(expectation, measured) {
  ...
  for (const spec of expectation.nodes || []) {
    ...
    for (const f of FIELDS) {
      ...
      if (bad) {
        deltas.push({
          ...
          delta: bad.delta,
          unit: f.unit,
          token: spec.tokens ? Object.values(spec.tokens)[0] : void 0
        });
      }
    }
    if (spec.padding !== void 0 && got.padding !== void 0) {
      fieldsChecked++;
    ...
    if (spec.text !== void 0 && got.text !== void 0) {
      fieldsChecked++;
      ...
      if (w !== g) {
        ...
        deltas.push({
          ...
          expected: spec.text,
          actual: got.text,
          note: caseOnly ? "differs only in case \u2014 check for a text-transform, which Figma applies at render time while storing the original" : void 0
        });
      }
  ...
  return {
    ...
    measuredAt: measured && measured.measuredAt || (/* @__PURE__ */ new Date()).toISOString(),
...
function reportToMarkdown(r) {
  ...
  L.push(`**${mark}** \xB7 renderer ${r.renderer}${r.viewport ? ` at ${r.viewport}` : ""} \xB7 measured ${r.measuredAt}`, "");
  if (r.why.length) {
...
` + USAGE);
  ...
  console.error(`${rep.verdict.toUpperCase()} \u2014 ${rep.summary.high} high, ${rep.summary.medium} medium, ${rep.summary.missingComponents} component(s) missing, ${rep.coverage.interactionsPassed}/${rep.coverage.interactionsExpected} interactions confirmed`);
