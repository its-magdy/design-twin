...
server.registerTool(
  "figma_write",
  {
    ...
    inputSchema: {
      ...
      ops: z.array(z.object({ op: z.enum(["createFrame", "createText", "setFill", "setText"]) }).passthrough()).max(50, "at most 50 ops per call \u2014 split a larger job into several calls (writes cannot be undone from here)").describe("Ordered list of write operations (each must have an `op` field). At most 50 per call."),
