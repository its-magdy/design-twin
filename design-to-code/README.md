...
| File | What it does |
|---|---|
| `design-diff.js` | What changed between two exports of one screen (or token file), matched by node **id**: changed fields with before → after, added/removed subtrees (top-most node only), reordered children, token values per mode. Position-only movement is counted, not listed. `--snapshot` keeps the previous export before a re-pull; without one it diffs against git `HEAD`. Drives the `sync-design` skill. |
