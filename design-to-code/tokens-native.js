...
function swiftui(cols) {
  ...
  for (const c of cols) {
    ...
    L.push("}", "", `private struct ${c.type}Key: EnvironmentKey { static let defaultValue = ${c.type}.${defId} }`,
      "public extension EnvironmentValues {", `    // Set once near the root: .environment(\\.${env}, colorScheme == .dark ? .dark : .light); read with @Environment(\\.${env}).`,
...
function reactNative(cols) {
  ...
  const lit = (k, r) => k === "color" ? str("#" + normHex(r)) : k === "bool" ? String(r === true || r === "true") : k === "string" ? str(r) : num(r);
  ...
  for (const c of cols) {
    ...
    L.push(`export interface ${c.type} {`, ...c.fields.map((f) => `  ${f.id}: ${T[f.kind]}; // ${f.source}`), "}", "",
      `export type ${c.type}Mode = ${c.modes.map((m) => str(m.id)).join(" | ")};`, `export const ${v}DefaultMode: ${c.type}Mode = ${str(c.modes.find((m) => m.name === c.default).id)};`, "",
